...
import { join, extname, basename, relative, dirname, parse } from 'path'
import fs from 'fs'
...
function rowToItem(row: ItemRow): LibraryItem {
  return {
    ...
    size: row.size,
    storedPath: join(filesDir, row.rel_path),
...
function registerFile(relPath: string, originalPath: string): LibraryItem | null {
  const storedPath = join(filesDir, relPath)
...
function importFile(srcPath: string): LibraryItem | null {
  try {
    ...
    return registerFile(relative(filesDir, dest), srcPath)
  } catch (err) {
...
function renameItem(id: string, newNameRaw: string): LibraryItem | null {
  ...
  ).run(basename(dest), ext, detectType(ext), relative(filesDir, dest), id)
...
function migrateFromJson(): void {
  ...
  try {
    ...
    for (const old of legacy) {
      ...
      const item = registerFile(relative(filesDir, dest), old.originalPath)
