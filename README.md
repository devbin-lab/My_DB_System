...
그래프 시각화가 핵심이라, 관련 자료구조와 알고리즘을 직접 구현했습니다.

- **그래프 표현** — 정점과 관계를 SQLite의 엣지 테이블(인접 리스트)로 저장합니다. 피벗↔파일, 파일↔파일은 무방향, 피벗↔피벗은 부모→자식 방향 엣지로 모델링합니다. 런타임에서는 노드 배열과 인덱스 쌍 엣지, `id → 인덱스` 해시맵으로 다룹니다.
- **레이아웃(force-directed)** — 반발력·스프링 인력·중심 중력에 시뮬레이티드 어닐링식 냉각(alpha decay)을 더한 물리 시뮬레이션을 매 프레임 계산합니다.
- **반발력 최적화(Barnes-Hut)** — 매 프레임 노드 위치로 쿼드트리를 만들고, 멀리 떨어진 노드 무리를 질량 중심 하나로 근사하여 반발력 계산을 O(n²)에서 **O(n log n)** 으로 낮췄습니다.
- **그래프 탐색(BFS)** — 부모 피벗을 열면 자식 방향으로만 너비 우선 탐색해 하위 서브트리를 펼치고, 호버 시 자손 전체를 모아 강조합니다.
- **레이아웃 안정화** — 노드 위치를 캐시해 재구성·화면 전환에도 배치를 보존하고, 변경 규모에 따른 약한 재가열과 프레임당 이동량 제한으로 요동을 억제합니다.
- **분류·조회** — 확장자 분류와 설정·다국어 조회는 해시셋/해시맵 기반 O(1), 노드 적중 판정은 거리 제곱 비교의 선형 스캔으로 처리합니다.
- **검색·정렬** — 라이브러리 목록은 선형 필터 후 이름/크기/날짜 비교 정렬, 그래프 검색 결과는 동심원 링에 극좌표로 배치합니다.
